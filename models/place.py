#!/usr/bin/python
"""Holds the Place class definition."""
import models
...
class Place(BaseModel, Base):
    ...
    if models.storage_t != 'db':
        @property
        def reviews(self):
            """Getter attribute that returns the list of Review instances associated with this Place."""
            from models.review import Review
            return [review for review in models.storage.all(Review).values() if review.place_id == self.id]

        @property
        def amenities(self):
            """Getter attribute that returns the list of Amenity instances associated with this Place."""
